...
func main() {
	...
	router.GET("/user/:name", func(cc *gin.Context) {
		err = c.Find(bson.M{"name": "Tracy Yu"}).All(&users)
		user1 := users[0];
		cc.String(http.StatusOK, "Hello %s", user1)
	})

	// However, this one will match /user/john/ and also /user/john/send
	// If no other routers match /user/john, it will redirect to /user/john/
	router.GET("/user/:name/*action", func(cc *gin.Context) {
		name := cc.Param("name")
		action := cc.Param("action")
		message := name + " is " + action
		cc.String(http.StatusOK, message)
	})
